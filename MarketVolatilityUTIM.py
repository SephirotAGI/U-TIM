...
class UniversalTIM(ABC):
    ...
    def _integrand(self, x, θ):
        """Integrand function combining weights, model discrepancy, and coherence factor."""
        return self.w(x, θ) * np.exp(-self.dC_dt(x, θ)) * np.linalg.norm(self.f_i(x, θ) - self.f_r(x))
...
if __name__ == "__main__":
    ...
    utim = EconomicsTIM(model, reference, PosteriorSampler(0.5, 0.1), lambda x, θ: 1.0, config=config)
