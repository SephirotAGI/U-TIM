...
if __name__ == "__main__":
    ...
    # Define an ecology-specific U-TIM implementation
    class EcologyTIM(UniversalTIM):
        def dC_dt(self, x, theta):
            return np.abs(theta - x / 100)  # Population dynamics model

    # Run U-TIM computation
    utim = EcologyTIM(model, reference, PosteriorSampler(0.5, 0.1), lambda x, θ: 1.0, config=config)
    ...
    if result.needs_human_review(0.15):
